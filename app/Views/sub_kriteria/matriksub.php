<?php
echo '<span class="badge badge-success">'.$namakriteria.'</span><br/><br/>';
$irdata=array(
1=>0.00,
2=>0.00,
3=>0.58,
...
<div id="entri">
<?php helper('form');
form_open('#',array('class'=>'form-horizontal','id'=>'formentri'));
?>
		...
		for($i=1;$i<=$jumlah;$i++)
		{
			...
			if($noSub==$noUtama)
			{
				...
				{									
					...
					echo '<td>';
                    $sub_k_model = new \App\Models\SubKriteriaModel();
					...
					for($x=1;$x<=9;$x++)
					{
						$nilai=$sub_k_model->ambil_nilai_subkriteria($kriteriaid,$k2,$xxx);
						$sl='';
						if($nilai==$x)
						{
	...
	<form style="display:inline-block" action="<?=base_url("")?>Sub_Kriteria/simpan_prioritas" method="post" accept-charset="utf-8">
		<?php
			...
			foreach($arrs as $k4=>$v4)
			{
				...
				echo '<input type="hidden" class="form-control" name="perioritas[]" id="prisubnilai-b'.$noUtama4.'"/>';
			}
		?>
